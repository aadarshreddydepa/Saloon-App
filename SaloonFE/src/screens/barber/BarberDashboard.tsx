...
export default function BarberDashboard() {
  ...
  const [filter, setFilter] = useState<"unassigned" | "my" | "all">(
    "unassigned"
  );
          ...
          onPress: async () => {
            try {
              ...
              if (!barberId) {
                Alert.alert(
                  "Error",
                  "Barber information not found. Please refresh."
                );
                return;
              }
              ...
              Alert.alert(
                "Error",
                error.response?.data?.error || "Failed to assign booking"
              );
            }
          },
        },
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                "Success",
                successMessages[status] || "Status updated"
              );
            } catch (error) {
  ...
  const getFilteredBookings = () => {
    ...
    if (filter === "unassigned") {
      return allBookings.filter(
        (b: any) =>
          !b.barber && b.status === "pending" && b.booking_date >= today
      );
    } else if (filter === "my") {
      return allBookings.filter(
        (b: any) =>
  ...
  const stats = {
    unassigned: allBookings.filter(
      (b: any) => !b.barber && b.status === "pending"
    ).length,
  ...
  const StatCard = ({ title, value, icon, color, onPress }: any) => (
    ...
    >
      ...
      <Text style={[styles.statTitle, { color: theme.text, opacity: 0.6 }]}>
        {title}
      </Text>
    </TouchableOpacity>
  );
        ...
        {barberInfo?.salon ? (
          <>
            ...
            >
              ...
              <View style={styles.salonInfo}>
                <Text style={[styles.salonLabel, { color: "#4CAF50" }]}>
                  Working At
                </Text>
                <Text style={[styles.salonName, { color: theme.text }]}>
                  {barberInfo.salon_name}
                </Text>
              </View>
            </View>
            ...
            {stats.unassigned > 0 && (
              ...
              >
                ...
                <Text style={[styles.alertText, { color: "#FF9800" }]}>
                  {stats.unassigned} new booking
                  {stats.unassigned > 1 ? "s" : ""} waiting for assignment!
                </Text>
              </View>
            )}
            ...
            >
              <TouchableOpacity
                style={[
                  styles.filterTab,
                  filter === "unassigned" && { backgroundColor: "#FF9800" },
                ]}
                onPress={() => setFilter("unassigned")}
              >
                <Text
                  style={[
                    styles.filterText,
                    { color: filter === "unassigned" ? "#FFF" : theme.text },
                  ]}
                >
              ...
              <TouchableOpacity
                style={[
                  styles.filterTab,
                  filter === "my" && { backgroundColor: "#2196F3" },
                ]}
                onPress={() => setFilter("my")}
              >
                <Text
                  style={[
                    styles.filterText,
                    { color: filter === "my" ? "#FFF" : theme.text },
                  ]}
                >
                      ...
                      <View style={styles.bookingMainInfo}>
                        ...
                        {booking.barber === barberInfo?.id && (
                          <View style={styles.assignedToMeBadge}>
                            <Text style={styles.assignedToMeText}>
                              Your Job
                            </Text>
                          </View>
                        )}
          ...
          >
            ...
            <View style={styles.joinSalonInfo}>
              <Text style={[styles.joinSalonTitle, { color: theme.text }]}>
                Join a Salon
              </Text>
              <Text
                style={[
                  styles.joinSalonSubtitle,
            ...
            {selectedBooking && (
              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.modalBody}>
                  ...
                  {(selectedBooking.barber === null ||
                    selectedBooking.barber === undefined ||
                  ...
                  {selectedBooking.barber === barberInfo?.id && (
                    <View style={styles.actionsSection}>
                      <Text
                        style={[styles.actionsTitle, { color: theme.text }]}
                      >
                        Update Status
                      </Text>
...
const styles = StyleSheet.create({
  ...
  greeting: { fontSize: 16, opacity: 0.7, fontFamily: fonts.body.regular },
  userName: { fontSize: 28, fontFamily: fonts.heading.bold, marginTop: 4 },
  notificationButton: {
    width: 45,
    height: 45,
    borderRadius: 22.5,
  ...
  salonName: { fontSize: 18, fontFamily: fonts.heading.bold },
  joinSalonCard: {
    marginHorizontal: 20,
    marginTop: 100,
    padding: 30,
  ...
  joinSalonSubtitle: { fontSize: 14, fontFamily: fonts.body.regular },
  ...
  statValue: { fontSize: 24, fontFamily: fonts.heading.bold, marginBottom: 4 },
  statTitle: {
    fontSize: 12,
    textAlign: "center",
  ...
  timeText: { fontSize: 13, marginLeft: 5, fontFamily: fonts.body.regular },
  price: { fontSize: 17, fontFamily: fonts.heading.bold },
  ...
  modalTitle: { fontSize: 22, fontFamily: fonts.heading.bold },
  ...
  detailValue: { fontSize: 16, fontFamily: fonts.body.semiBold },
  acceptButton: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
