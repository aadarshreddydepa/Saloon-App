...
export default function JoinSalonRequest() {
  ...
  const handleSendRequest = async (salon: any) => {
    if (currentBarberInfo?.salon) {
      Alert.alert(
        "Already Joined",
        "You are already part of a salon. You can only join one salon at a time."
      );
      return;
    }
          ...
          onPress: async () => {
            try {
              await barberAPI.sendJoinRequest(salon.id, {
                ...
                } and I'd like to join your salon.`,
              });
...
const styles = StyleSheet.create({
  ...
  salonInfo: { flex: 1 },
  salonName: { fontSize: 17, fontFamily: fonts.heading.bold, marginBottom: 4 },
  salonAddress: {
    fontSize: 13,
    marginBottom: 6,
    fontFamily: fonts.body.regular,
