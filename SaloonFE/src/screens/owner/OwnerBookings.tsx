...
export default function OwnerBookings() {
  ...
  const [filter, setFilter] = useState<'all' | 'unassigned' | 'dateRange'>('all');
  ...
  const theme = isDark 
    ? { bg: '#000000', card: '#1A1A1A', text: '#C0C0C0', primary: '#C0C0C0', accent: '#2196F3', border: '#333' }
    : { bg: '#F5F5F5', card: '#FFFFFF', text: '#000000', primary: '#000000', accent: '#2196F3', border: '#E0E0E0' };
    ...
    return (
      <View style={[styles.bookingCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
        <View style={styles.cardContent}>
          {/* Left: Date Badge */}
          <View style={[styles.dateBadge, { backgroundColor: getStatusColor(item.status) }]}>
          ...
          <View style={styles.bookingDetails}>
            ...
            <View style={styles.serviceRow}>
              ...
              <Text style={[styles.price, { color: theme.accent }]}>₹{item.service_price}</Text>
            </View>

            <View style={styles.timeBarberRow}>
              <View style={styles.timeInfo}>
                <Ionicons name="time" size={14} color={theme.text + '80'} />
                <Text style={[styles.timeText, { color: theme.text }]}>{item.booking_time}</Text>
              </View>
              
              {item.barber_name ? (
                <View style={styles.barberBadge}>
                  <Ionicons name="person" size={12} color="#4CAF50" />
                  <Text style={styles.barberText} numberOfLines={1}>{item.barber_name}</Text>
                </View>
              ) : (
                <View style={[styles.barberBadge, { backgroundColor: '#FF980015' }]}>
                  <Ionicons name="alert-circle" size={12} color="#FF9800" />
                  <Text style={[styles.barberText, { color: '#FF9800' }]}>Unassigned</Text>
                </View>
              )}
            </View>
          </View>
    ...
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={[styles.filterBar, { backgroundColor: theme.card }]}>
        ...
        >
          ...
          <Text style={[styles.compactDate, { color: theme.accent }]}>
            {formatDisplayDate(fromDate).month} {formatDisplayDate(fromDate).day}
          </Text>
        </TouchableOpacity>
      ...
      <View style={[styles.inlineStats, { backgroundColor: theme.card }]}>
        <TouchableOpacity 
          style={[
            styles.inlineStat,
            filter === 'all' && !dateRangeActive && { backgroundColor: theme.accent + '15' }
          ]}
          onPress={() => { setFilter('all'); setDateRangeActive(false); }}
        >
          <Ionicons name="albums" size={18} color="#2196F3" />
          <Text style={[styles.inlineStatValue, { color: theme.text }]}>{stats.total}</Text>
          <Text style={[styles.inlineStatLabel, { color: theme.text }]}>Total</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity 
          style={[
            styles.inlineStat,
            filter === 'unassigned' && !dateRangeActive && { backgroundColor: '#FF980015' }
          ]}
          onPress={() => { setFilter('unassigned'); setDateRangeActive(false); }}
        >
          <Ionicons name="alert-circle" size={18} color="#FF9800" />
          <Text style={[styles.inlineStatValue, { color: theme.text }]}>{stats.unassigned}</Text>
          <Text style={[styles.inlineStatLabel, { color: theme.text }]}>Unassigned</Text>
        </TouchableOpacity>

        {dateRangeActive && (
          <>
            <View style={[styles.statDivider, { backgroundColor: theme.border }]} />
            <View style={[styles.inlineStat, { backgroundColor: theme.accent + '15' }]}>
              <Ionicons name="funnel" size={18} color={theme.accent} />
              <Text style={[styles.inlineStatValue, { color: theme.text }]}>{filteredBookings.length}</Text>
              <Text style={[styles.inlineStatLabel, { color: theme.text }]}>Filtered</Text>
            </View>
          </>
        )}
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Ionicons name="calendar-outline" size={70} color={theme.text + '30'} />
            <Text style={[styles.emptyText, { color: theme.text }]}>No bookings found</Text>
            <Text style={[styles.emptySubtext, { color: theme.text }]}>
              {dateRangeActive ? 'Try adjusting date range' : 'Bookings will appear here'}
            </Text>
          </View>
        }
      />
