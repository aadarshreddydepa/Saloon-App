...
export default function ManageSalons() {
  ...
  const renderSalon = ({ item }: any) => (
    <TouchableOpacity
      ...
      onPress={() => navigation.navigate('SalonDetail' as never, { id: item.id })}
    >
      ...
      <View style={styles.salonInfo}>
        ...
        <View style={styles.salonMeta}>
          <View style={styles.rating}>
            ...
            <Text style={[styles.ratingText, { color: theme.text }]}>{item.rating || '0.0'}</Text>
          </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      <View style={[styles.header, { backgroundColor: theme.card }]}>
        <Text style={[styles.headerTitle, { color: theme.text }]}>My Salons</Text>
        <TouchableOpacity onPress={() => navigation.navigate('AddSalon' as never)}>
...
const styles = StyleSheet.create({
  ...
  salonIcon: { width: 70, height: 70, borderRadius: 15, justifyContent: 'center', alignItems: 'center', marginRight: 15 },
  salonInfo: { flex: 1 },
